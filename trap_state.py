...
#
# Generated Wed Dec 17 16:51:19 2014 by generateDS.py version 2.13a.
#
...
#
# Command line:
#   generateDS.py -o "../trap_state.py" ../trap_state.xsd
#
...
class trap(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, name=None, type_=None, energy=None, deg=None, sigmae=None, sigmah=None, eta=None):
        self.original_tagname_ = None
        self.name = name
        self.type_ = type_
        self.energy = energy
    ...
    def get_sigmae(self): return self.sigmae
    def set_sigmae(self, sigmae): self.sigmae = sigmae
    def get_sigmah(self): return self.sigmah
    def set_sigmah(self, sigmah): self.sigmah = sigmah
    ...
    def hasContent_(self):
        if (
            ...
            self.deg is not None or
            self.sigmae is not None or
            self.sigmah is not None or
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='trap', fromsubclass_=False, pretty_print=True):
        ...
        if self.energy is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%senergy>%s</%senergy>%s' % (namespace_, self.gds_format_double(self.energy, input_name='energy'), namespace_, eol_))
        if self.deg is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%sdeg>%s</%sdeg>%s' % (namespace_, self.gds_format_double(self.deg, input_name='deg'), namespace_, eol_))
        if self.sigmae is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%ssigmae>%s</%ssigmae>%s' % (namespace_, self.gds_format_double(self.sigmae, input_name='sigmae'), namespace_, eol_))
        if self.sigmah is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%ssigmah>%s</%ssigmah>%s' % (namespace_, self.gds_format_double(self.sigmah, input_name='sigmah'), namespace_, eol_))
        if self.eta is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%seta>%s</%seta>%s' % (namespace_, self.gds_format_double(self.eta, input_name='eta'), namespace_, eol_))
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        if self.energy is not None:
            showIndent(outfile, level)
            outfile.write('energy=%e,\n' % self.energy)
        if self.deg is not None:
            showIndent(outfile, level)
            outfile.write('deg=%e,\n' % self.deg)
        if self.sigmae is not None:
            showIndent(outfile, level)
            outfile.write('sigmae=%e,\n' % self.sigmae)
        if self.sigmah is not None:
            showIndent(outfile, level)
            outfile.write('sigmah=%e,\n' % self.sigmah)
        if self.eta is not None:
            showIndent(outfile, level)
            outfile.write('eta=%e,\n' % self.eta)
    def build(self, node):
    ...
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'name':
            ...
            fval_ = self.gds_validate_float(fval_, node, 'energy')
            self.energy = fval_
        elif nodeName_ == 'deg':
            sval_ = child_.text
            try:
                ...
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'deg')
            self.deg = fval_
        elif nodeName_ == 'sigmae':
            sval_ = child_.text
            try:
                fval_ = float(sval_)
            except (TypeError, ValueError), exp:
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'sigmae')
            self.sigmae = fval_
        elif nodeName_ == 'sigmah':
            sval_ = child_.text
            try:
                fval_ = float(sval_)
            except (TypeError, ValueError), exp:
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'sigmah')
            self.sigmah = fval_
        elif nodeName_ == 'eta':
            sval_ = child_.text
            try:
                ...
                raise_parse_error(child_, 'requires float or double: %s' % exp)
            fval_ = self.gds_validate_float(fval_, node, 'eta')
            self.eta = fval_
# end class trap
...
class model(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, name=None, trap=None):
        self.original_tagname_ = None
        self.name = name
        if trap is None:
            self.trap = []
...
def parseLiteral(inFileName, silence=False):
    ...
    doc = None
    if not silence:
        sys.stdout.write('#from trap_state import *\n\n')
        sys.stdout.write('import trap_state as model_\n\n')
