...
#
# Generated Fri Nov 14 19:24:35 2014 by generateDS.py version 2.13a.
#
# Command line options:
#   ('--super', 'superimpose')
#   ('-o', 'superimpose.py')
#   ('-s', 'superimposesubs.py')
#
# Command line arguments:
#   ../superimpose.xsd
#
# Command line:
#   generateDS.py --super="superimpose" -o "superimpose.py" -s "superimposesubs.py" ../superimpose.xsd
#
...
class curve(GeneratedsSuper):
    subclass = None
    superclass = None
    def __init__(self, data=None, label=None, marker=None, color=None):
        self.original_tagname_ = None
        self.data = data
        self.label = label
        self.marker = marker
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='curve', fromsubclass_=False, pretty_print=True):
        ...
        if self.marker is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%smarker>%s</%smarker>%s' % (namespace_, self.gds_format_string(quote_xml(self.marker).encode(ExternalEncoding), input_name='marker'), namespace_, eol_))
        if self.color is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write('<%scolor>%s</%scolor>%s' % (namespace_, self.gds_format_string(quote_xml(self.color).encode(ExternalEncoding), input_name='color'), namespace_, eol_))
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        if self.marker is not None:
            showIndent(outfile, level)
            outfile.write('marker=%s,\n' % quote_python(self.marker).encode(ExternalEncoding))
        if self.color is not None:
            showIndent(outfile, level)
            outfile.write('color=%s,\n' % quote_python(self.color).encode(ExternalEncoding))
    def build(self, node):
        already_processed = set()
        self.buildAttributes(node, node.attrib, already_processed)
        for child in node:
    ...
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'data':
            ...
            marker_ = child_.text
            marker_ = self.gds_validate_string(marker_, node, 'marker')
            self.marker = marker_
...
def parseLiteral(inFileName, silence=False):
    ...
    doc = None
    if not silence:
        sys.stdout.write('#from superimpose import *\n\n')
        sys.stdout.write('import superimpose as model_\n\n')
