...
def write_asymmetry_2D(filename1,filename2,savefile):
  ...
  for line in range(nlines):
    ...
    if ( tmp1_field1 == 0 and tmp2_field1 == 0 ):
      ...
      if (asymm > max_diff):
        print("lines #"+str(line+1)+" differ more than "+max_diff+": "+asymm)
        return -2
    ...
    assert(tmp1_field3+tmp2_field3)
    asymmetry = (tmp1_field3-tmp2_field3)/(tmp1_field3+tmp2_field3)
    output_file.write('%f %f %e\n' % (tmp1_field1, tmp1_field2, asymmetry))
...
if __name__ == "__main__":
  ...
  savefile  = sys.argv[3]
  write_asymmetry_2D(filename1,filename2,savefile)
